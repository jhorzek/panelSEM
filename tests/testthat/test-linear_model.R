test_that("test linear model", {
    ...
    for(heterogeneity in c("homogeneous", "additive")){
      for(use_open_mx in c(TRUE, FALSE)){
        ...
        if(use_open_mx){
          ...
          # remove intercepts
          coef_fit <- coef_fit[!grepl(pattern = "^mean_",
                                      x = coef_fit)]
        }else{
  ...
  z1 ~~ psi_z1_z1*z1 + psi_z1_z2*z2 + psi_z1_z3*z3
  z2 ~~ psi_z2_z2*z2 + psi_z2_z3*z3
  z3 ~~ psi_z3_z3*z3
        "
          )
        }
  ...
  etax ~~ psi_etax_etax * etax + psi_etax_etay * etay
